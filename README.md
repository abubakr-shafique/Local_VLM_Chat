...
Chat with open-source **vision-language**, **text**, and **coding** models — including image
...
- 🖼️ Image understanding with VLMs (upload via 📎 or paste directly with Ctrl+V — clipboard
  paste is handled natively by the browser, no source flag needed)
...
**Deliberately excluded (don't fit 16 GB even at 4-bit):** Qwen3.5-27B (~15 GB weights alone,
no room for KV cache), Qwen3.5-35B-A3B and Qwen3.6-35B-A3B (~19–20 GB), Gemma 4 26B, and all
...
the 📎 button or paste straight from the clipboard (Ctrl+V). Images are only processed when a
VLM is loaded; with a text/coding model they are replaced by a placeholder note.
...
`vllm serve ./models/Qwen3.5-9B` and point an OpenAI-compatible client at it.

## Adding a new model

1. Add an entry to `MODEL_REGISTRY` in `model_registry.py` (repo id, `kind`, `category`,
   VRAM estimate).
2. Run `python download_models.py --models "<YourKey>"`.
3. Select it in the UI. Any model whose architecture is supported by
   `AutoModelForImageTextToText` (VLMs) or `AutoModelForCausalLM` (text/coding) works out of
   the box.

## Troubleshooting

- **Qwen3.5 fails to load with an unrecognized-architecture error** → your transformers is
  too old: `pip install -U transformers`.
...
- **Qwen3-Coder-30B is very slow** → expected: it doesn't fit in 16 GB, so some layers run
  from system RAM. Use Qwen2.5-Coder-14B for full-GPU speed.
- **Qwen3 / Qwen3.5 output starts with a `<think>` block** → these are hybrid-reasoning
  models; the reasoning is streamed before the final answer.
...
(Qwen/Qwen3.5/Qwen-Coder: Apache 2.0; Mistral/Devstral: Apache 2.0; Gemma: Gemma Terms of Use;
Llama: Llama Community License) — review them before redistribution or commercial use.